...
out_dir = os.path.join(root_dir, 'demo/test')
...
def draw_vid(vid_name, vid_dir, person_dir, face_dir, pose_dir, out_dir, blur):
    ...
    out_name = vid_name[:-4].replace(' ', '_') + '_overlaytest.mp4'
...
if __name__ == "__main__":
    ...
    for vid_name in vid_names:
        ...
        if vid_id in TWs:
            blur = True
        draw_vid(name, vid_dir, person_dir, face_dir, pose_dir, out, blur)
